...
class ProductClass {
	...
	purchase(quantity) {
		if (this.quantity === 0) {
			...
			this.quantity -= quantity
			console.log(
				`You bought a product: ${this.name}. There are  ${this.quantity} ${this.name} left in stock.`,
			)
		} else {
			console.log(
				`There are not enough ${this.name} in stock to purchase. There are only ${this.quantity} ${this.name} left in stock.`,
			)
		}
	}
}
